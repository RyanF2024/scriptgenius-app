...
export function SocialConnections() {
  const { accounts, connectAccount, disconnectAccount, isLoading } = useSocial();
  ...
  const handleConnect = async (provider: SocialProvider) => {
    try {
      ...
      toast({
        ...
        description: 'Failed to connect the social account. Please try again.',
        type: 'error',
      });
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Manage your social account connections. You can connect multiple accounts to sign in.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        {Object.entries(providerConfigs).map(([provider, config]) => {
          const connected = isConnected(provider);
          const email = getAccountEmail(provider);
